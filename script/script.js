if (window.location.href === "http://127.0.0.1:5500/catalog.html") {
  (function openMap() {
    const Catalog = document.querySelector(".catalog-item-list");
    const cart = document.querySelector(".modal-cart");
    let cartAdd = document.querySelector(".cart-full");
    ...
    cart.addEventListener("click", (e) => {
      if (
        ...
        e.target.classList.contains("cart-continue")
      ) {
  ...
  (function formReset() {
    const form = document.querySelector(".form-item-filter");
    ...
    indicatorList.addEventListener("click", (e) => {
      ...
      if (slideNumber || slideIndicatorButton) {
        ...
        if (slideIndicatorButton) {
          const parentElementDataset =
            e.target.closest("li").dataset.slideNumber;
